...
function sortValidations() {
  ...
  var validations = vueDemi.unref(validationsRaw);
...
function createAsyncResult(rule, model, $pending, $dirty, _ref, $response, instance) {
  ...
  var $invalid = vueDemi.ref(!!$dirty.value);
  var $pendingCounter = vueDemi.ref(0);
  $pending.value = false;
  var $unwatch = vueDemi.watch([model, $dirty].concat(watchTargets), function () {
...
function createSyncResult(rule, model, $dirty, _ref2, $response, instance) {
  ...
  var $invalid = vueDemi.computed(function () {
    if ($lazy && !$dirty.value) return false;
...
function createValidatorResult(rule, model, $dirty, config, instance, validatorName, propertyKey, propertyPath) {
  var $pending = vueDemi.ref(false);
  var $params = rule.$params || {};
  var $response = vueDemi.ref(null);
  var $invalid;
  var $unwatch;
  ...
  var $message = isFunction(message) ? vueDemi.computed(function () {
...
function createValidationResults(rules, model, key, resultsCache, path, config, instance, externalResults) {
  ...
  result.$externalResults = vueDemi.computed(function () {
  ...
  result.$invalid = vueDemi.computed(function () {
    return !!result.$externalResults.value.length || ruleKeys.some(function (ruleKey) {
      return vueDemi.unref(result[ruleKey].$invalid);
    });
  });
  result.$pending = vueDemi.computed(function () {
    return ruleKeys.some(function (ruleKey) {
      return vueDemi.unref(result[ruleKey].$pending);
    });
  });
  result.$error = vueDemi.computed(function () {
    return result.$invalid.value && result.$dirty.value;
  });
  result.$silentErrors = vueDemi.computed(function () {
  ...
  result.$errors = vueDemi.computed(function () {
...
function createMetaFields(results, nestedResults, childResults) {
  var allResults = vueDemi.computed(function () {
  ...
  var $silentErrors = vueDemi.computed(function () {
    // current state level errors, fallback to empty array if root
    var modelErrors = vueDemi.unref(results.$silentErrors) || []; // collect all nested and child $silentErrors

    var nestedErrors = allResults.value.filter(function (result) {
      return (vueDemi.unref(result).$silentErrors || []).length;
  ...
  var $errors = vueDemi.computed(function () {
    // current state level errors, fallback to empty array if root
    var modelErrors = vueDemi.unref(results.$errors) || []; // collect all nested and child $errors

    var nestedErrors = allResults.value.filter(function (result) {
      return (vueDemi.unref(result).$errors || []).length;
    }).reduce(function (errors, result) {
  ...
  var $error = vueDemi.computed(function () {
...
function setValidations(_ref3) {
  ...
  var nestedState = key ? vueDemi.computed(function () {
    var s = vueDemi.unref(state);
    return s ? vueDemi.unref(s[key]) : undefined;
  }) : state; // cache the external results, so we can revert back to them

  var cachedExternalResults = Object.assign({}, vueDemi.unref(externalResults) || {});
  var nestedExternalResults = vueDemi.computed(function () {
    var results = vueDemi.unref(externalResults);
  ...
  var $model = key ? vueDemi.computed({
    get: function get() {
      return vueDemi.unref(nestedState);
    },
  ...
  if (key && mergedConfig.$autoDirty) {
    var $unwatch = vueDemi.watch(nestedState, function () {
  ...
  function $validate() {
    return new Promise(_async(function (resolve) {
      ...
      return _call(vueDemi.nextTick, function () {
        ...
        var unwatch = vueDemi.watch($pending, function () {
...
function nestedValidations(_ref) {
  ...
  var childResultsRaw = {};
  var childResultsKeys = vueDemi.ref([]);
  var childResults = vueDemi.computed(function () {
  ...
  var sendValidationResultsToParent = vueDemi.inject(VuelidateInjectChildResults, function () {}); // provide to all of its children the send results to parent function

  vueDemi.provide(VuelidateInjectChildResults, injectChildResultsIntoParent);
  var removeValidationResultsFromParent = vueDemi.inject(VuelidateRemoveChildResults, function () {}); // provide to all of its children the remove results  function

  vueDemi.provide(VuelidateRemoveChildResults, removeChildResultsFromParent);
  return {
...
function useVuelidate(validations, state) {
  ...
  var componentOptions = instance ? vueDemi.isVue3 ? instance.type : instance.proxy.$options : {}; // if there is no registration name, add one.
  ...
  var validationResults = vueDemi.ref({});
  ...
  if (!validations && componentOptions.validations) {
    ...
    state = vueDemi.ref({});
    vueDemi.onBeforeMount(function () {
      ...
      function ComputedProxyFactory(target) {
        return new Proxy(target, {
          get: function get(target, prop, receiver) {
            return _typeof(target[prop]) === 'object' ? ComputedProxyFactory(target[prop]) : vueDemi.computed(function () {
              return target[prop];
            });
          }
        });
    ...
    var validationsWatchTarget = vueDemi.isRef(validations) || isProxy(validations) ? validations // wrap plain objects in a reactive, so we can track changes if they have computed in them.
    : vueDemi.reactive(validations || {});
    vueDemi.watch(validationsWatchTarget, function (newValidationRules) {
      validationResults.value = setValidations({
